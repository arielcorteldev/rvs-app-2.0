...
class ManualMarriageEntryCard(QFrame):
    ...
    def __init__(self, current_user, parent=None):
        ...
        self.husband_name_input.editingFinished.connect(self._check_for_existing_record)
        self.wife_name_input.editingFinished.connect(self._check_for_existing_record)
    ...
    def _check_for_existing_record(self):
        ...
        Wife Name are all filled in, check whether a matching marriage_index
        row already exists. Requires all three (not just Reg. No. + one
        name) to reduce the odds of a false match, same as the reconciliation
        check on the scanned-tagging side.
        ...
        - If it's another manual (unscanned) entry: open that one instead
          of creating a second one for the same couple.
        """
        ...
        try:
            ...
            cursor.execute("""
                SELECT id, scanned
                FROM marriage_index
                WHERE reg_no = %s AND husband_name ILIKE %s AND wife_name ILIKE %s
                LIMIT 1
        ...
        if existing_scanned:
            ...
            box.setText(
                f"A record for Reg. No. {reg_no} — {husband_name} & {wife_name} has "
                "already been scanned and tagged. It's already searchable and can "
                "generate an LCR certificate through Verify — there's no need to "
                "create a manual entry for it. This window will now close."
            )
        ...
        box.setText(
            ...
            f"{husband_name} & {wife_name}. Opening that record for editing."
        )
        box.setStandardButtons(QMessageBox.Ok)
        box.setStyleSheet(message_box_style)
        box.exec()
