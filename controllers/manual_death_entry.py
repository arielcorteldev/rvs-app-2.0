"""ManualDeathEntryWindow — standalone window for tagging a Death record
that has not been scanned yet.

Opened from verify.py's create_form button (Death window) in place of the
old blank-PDF-form workflow. Cached/reused via app.py's self.windows
pattern, same as every other window in the app. Uses ManualDeathEntryCard
(utilities/manual_death_entry_card.py) for the actual fields — this file
just hosts it in a window and owns the reminder pop-up / close behavior.

Mirrors controllers/manual_birth_entry.py exactly.
"""
...
from utilities.manual_death_entry_card import ManualDeathEntryCard


class ManualDeathEntryWindow(QWidget):
    """Standalone window hosting a single ManualDeathEntryCard.

    Opened from verify.py's create_form button (Death window) in place of
    ...
    def __init__(self, username, parent=None, main_window=None):
        ...
        self.setWindowTitle("Manual Entry — Death")
    ...
    def _build_ui(self):
        ...
        info_label = QLabel(
            "Use this form to encode/tag the details of a Death record that has not been scanned yet. "
            "Onced saved, this record will be searchable through the Verify menu and an auto-populated LCR "
            "certificate can be generated for this record."
        )
        ...
        self.card = ManualDeathEntryCard(current_user=self.current_user)
