"""ManualMarriageEntryWindow — standalone window for tagging a Marriage
record that has not been scanned yet.

Opened from verify.py's create_form button (Marriage window) in place of
the old blank-PDF-form workflow. Cached/reused via app.py's self.windows
pattern, same as every other window in the app. Uses
ManualMarriageEntryCard (utilities/manual_marriage_entry_card.py) for the
actual fields — this file just hosts it in a window and owns the reminder
pop-up / close behavior.

Mirrors controllers/manual_birth_entry.py and manual_death_entry.py exactly.
"""
...
from utilities.manual_marriage_entry_card import ManualMarriageEntryCard


class ManualMarriageEntryWindow(QWidget):
    """Standalone window hosting a single ManualMarriageEntryCard.

    Opened from verify.py's create_form button (Marriage window) in place
    of the old blank-PDF-form workflow. Cached/reused via app.py's
    self.windows pattern, same as every other window in the app.
    """
    ...
    def __init__(self, username, parent=None, main_window=None):
        ...
        self.setWindowTitle("Manual Entry — Marriage")
    ...
    def _build_ui(self):
        ...
        self.card = ManualMarriageEntryCard(current_user=self.current_user)
